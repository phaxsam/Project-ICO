...
contract ICO is ReentrancyGuard {
    ...
    modifier endOfICO() {
        require(end > 0 && (block.timestamp >= end || availableTokens == 0), "ICO must END");
        _;
    }
    modifier onlyAdmin() {
    ...
    function start(uint256 duration, uint256 _price, uint256 _availableTokens, uint256 _minPurchase, uint256 _maxPurchase) external onlyAdmin unactivatedICO {
    ...
    function release() external onlyAdmin endOfICO tokensNotReleased nonReentrant {
    ...
    function withdraw(address payable to, uint256 amount) external onlyAdmin endOfICO tokensReleased nonReentrant {
        to.transfer(amount);
    }
